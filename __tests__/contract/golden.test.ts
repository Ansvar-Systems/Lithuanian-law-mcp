...
// ---------------------------------------------------------------------------
// Skip entire suite when database is not available (CI without large assets)
// ---------------------------------------------------------------------------

const DB_PATH =
  process.env['LITHUANIAN_LAW_DB_PATH'] ?? join(__dirname, '..', '..', 'data', 'database.db');
...
describe.skipIf(!DB_AVAILABLE)(`Contract tests: ${fixture.mcp_name}`, () => {
  beforeAll(async () => {
    // node-sqlite3-wasm leaves a .lock dir on close — remove stale lock before opening
    try {
      rmdirSync(DB_PATH + '.lock');
    } catch {
      /* ignore */
    }
