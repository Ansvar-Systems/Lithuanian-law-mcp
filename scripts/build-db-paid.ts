#!/usr/bin/env tsx
/**
 * Paid-tier database builder for Lithuanian Law MCP server.
 *
 * ADDITIVE -- does NOT rebuild from scratch.
 *
 * Usage: npx tsx scripts/build-db-paid.ts
 *        npx tsx scripts/build-db-paid.ts --db /path/to/database.db
 */
...
const DEFAULT_DB_PATH = path.resolve(__dirname, '../../../Lithuanian-law-mcp/data/database.db');
...
CREATE TABLE IF NOT EXISTS case_law (
  ...
  source TEXT NOT NULL DEFAULT 'liteko',
...
CREATE TABLE IF NOT EXISTS preparatory_works (
  ...
  committee TEXT,
  proposer TEXT,
  source TEXT NOT NULL DEFAULT 'e_seimas',
  inserted_at TEXT DEFAULT (datetime('now'))
);
...
function buildPaidTier(): void {
  ...
  console.log('Building paid-tier extensions for Lithuanian Law MCP...\n');
  ...
  const updateMeta = db.transaction(() => {
    ...
    upsertMeta.run('jurisdiction', 'LT');
    upsertMeta.run('paid_tables', 'case_law,case_law_full,preparatory_works,preparatory_works_full,agency_guidance');
    upsertMeta.run('source_case_law', 'liteko.teismai.lt');
    upsertMeta.run('source_prep_works', 'e-seimas.lrs.lt');
    upsertMeta.run('source_agency', 'ada.lt,lrv.lt');
  });
  updateMeta();
