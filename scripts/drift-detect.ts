#!/usr/bin/env tsx
/**
 * Drift detection for Lithuanian Law MCP.
 *
 * Checks if upstream www.e-tar.lt content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Lithuanian Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Lithuanian-Law-MCP/1.0 drift-detect' },
      });
