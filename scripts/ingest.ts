#!/usr/bin/env tsx
/**
 * Lithuanian Law MCP -- Ingestion Pipeline
 *
 * Fetches Lithuanian legislation from the Sejm ELI API (api.sejm.gov.pl).
 * The Sejm (Lithuanian Parliament) provides free public access to all legislation
 ...
 * License: Lithuanian legislation is public domain under Art. 4 of the Copyright Act
 */
...
import { parseLithuanianHtml, KEY_LITHUANIAN_ACTS, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function fetchAndParseActs(acts: ActIndexEntry[], skipFetch: boolean): Promise<void> {
  console.log(`\nProcessing ${acts.length} Lithuanian Acts from api.sejm.gov.pl...\n`);
  ...
  for (const act of acts) {
    ...
    try {
      ...
      const parsed = parseLithuanianHtml(html, act);
  ...
  console.log(`  License:     Public domain (Art. 4 Lithuanian Copyright Act)`);
...
async function main(): Promise<void> {
  ...
  console.log('Lithuanian Law MCP -- Ingestion Pipeline');
  console.log('====================================\n');
  ...
  const acts = limit ? KEY_LITHUANIAN_ACTS.slice(0, limit) : KEY_LITHUANIAN_ACTS;
