#!/usr/bin/env tsx
/**
 * Lithuanian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.e-tar.lt';
const PORTAL_NAME = 'TAR (Teises aktu registras)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/lithuanian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Lithuanian Law MCP — Data Freshness Check');
