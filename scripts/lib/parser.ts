/**
 * HTML parser for Lithuanian legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Lithuanian legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Lithuanian statute.
 *
 ...
 *   </div>
 */
export function parseLithuanianHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Lithuanian acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Lithuanian legal text.
 *
 * Lithuanian definitions typically use patterns like:
