/**
 * get_lithuanian_implementations — Find Lithuanian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetLithuanianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface LithuanianImplementationResult {
  document_id: string;
...
export async function getLithuanianImplementations(
  db: InstanceType<typeof Database>,
  input: GetLithuanianImplementationsInput,
): Promise<ToolResponse<LithuanianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as LithuanianImplementationResult[];
