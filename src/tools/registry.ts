/**
 * Tool registry for Lithuanian Law MCP Server.
...
import { getLithuanianImplementations, type GetLithuanianImplementationsInput } from './get-lithuanian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the TAR (Teisės aktų registras) (Lithuanian Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Lithuanian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Lithuanian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Lithuanian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Lithuanian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Lithuanian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Lithuania?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Lithuanian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Lithuanian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Lithuanian statute references or aligns with. ' +
      'Lithuania is an EU member state and transposes EU directives into national law ' +
      '(e.g., ADTAĮ implements GDPR, KSĮ implements NIS2). ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Lithuanian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_lithuanian_implementations',
    description:
      'Find all Lithuanian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Lithuanian statutes. ' +
      'Lithuania transposes EU directives into national law as an EU member state.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Lithuanian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_lithuanian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Lithuanian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Lithuanian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Lithuanian statute or provision. ' +
      ...
      'Lithuania is bound by EU law as a member state; this checks implementation and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Lithuanian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_lithuanian_implementations':
          result = await getLithuanianImplementations(db, args as unknown as GetLithuanianImplementationsInput);
          break;
