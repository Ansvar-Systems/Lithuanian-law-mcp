/**
 * search_eu_implementations — Search EU directives/regulations referenced by Lithuanian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_lithuanian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  lithuanian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as lithuanian_statute_count
  ...
  if (input.has_lithuanian_implementation) {
    sql += ' HAVING lithuanian_statute_count > 0';
  }
